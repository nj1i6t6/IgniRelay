...
### 17.2 Coverage (envelope slice)

- ≥ 100 `(envelope_struct, expected_canonical_bytes_hex, expected_proto_bytes_hex, expected_signature_hex)` samples.
- ≥ 1 sample per EventType.
- ≥ 1 sample per Priority.
- ≥ 5 SOS_RED samples that fit the 240 B envelope cap (one each at ~150 B, ~190 B, ~220 B, ~235 B, exactly 240 B).
- ≥ 5 ALERT samples that REQUIRE chunking (size > single-notify capacity but ≤ 800 B).
- ≥ 1 RESOURCE sample at exactly the 400 B cap.
...
- Envelope with `sig_algo = 0x02` (unknown).
- Envelope where `payload` bytes were tampered between sender and receiver (signature verification fails because locally computed `SHA-256(payload)` no longer matches the signed input).
- Envelope with `priority = SOS_RED` AND `event_type = CHAT_MESSAGE`.
- Envelope with `event_type = STATUS_UPDATE` AND `priority = NORMAL` (matrix §6 says downgrade to STATUS — receiver test verifies downgrade behavior, not drop).
- Over-budget SOS_RED (total_size > 240B).
- Over-budget RESOURCE (> 400B).
- Over-budget NORMAL (> 2048B).
- Envelope with `is_experimental = false` AND `event_type = 1500` (unknown non-experimental).
- Envelope with `max_hops` exceeding default for its event_type (§11.3).

### 17.4 CI gate

Each of Dart / Kotlin / Swift MUST encode and decode every positive sample bit-identically (proto bytes AND canonical signature bytes), and reject every negative sample with the documented `drop_reason`. CI is part of the Stage 0c acceptance check (brief §3.4).

### 17.5 Test vector generation tooling

A Dart-only `tool/generate_wire_conformance_v1.dart` script produces the JSON from a set of hand-authored YAML scenarios. It is the ONLY source of truth for new test vectors. Kotlin and Swift implementations are CONSUMERS of the JSON; they MUST NOT regenerate the corpus.

The script lives at `resqmesh_app/tool/generate_wire_conformance_v1.dart`. Its inputs (YAML scenarios) live under `resqmesh_app/test/wire_conformance/scenarios/`. Its output (the JSON) is committed to `docs/specs/wire_conformance_v1.json`.
