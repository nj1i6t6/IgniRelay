...
function WritePOI(class,subclass,rank)
	...
	-- ResQMesh: 額外寫入聯絡/營業資訊供 App 彈窗查詢
	local phone = Find("phone")
	if phone=="" then phone = Find("contact:phone") end
	if phone~="" then Attribute("phone", phone) end
	local hours = Find("opening_hours")
	if hours~="" then Attribute("opening_hours", hours) end
	local housenumber = Find("addr:housenumber")
	if housenumber~="" then Attribute("housenumber", housenumber) end
	local street = Find("addr:street")
	if street~="" then Attribute("addr_street", street) end
end
